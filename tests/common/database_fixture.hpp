...
extern uint32_t ( MUSE_TESTING_GENESIS_TIMESTAMP );

#define PUSH_TX \
   muse::chain::test::_push_transaction

#define PUSH_BLOCK \
   muse::chain::test::_push_block
...
#define MUSE_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "MUSE_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "MUSE_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}

#define MUSE_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "MUSE_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "MUSE_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   MUSE_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   MUSE_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
struct database_fixture {
   ...
   void transfer( const string& from, const string& to, const share_type& muse );
