   ...
   ) const
{
   ...
   for( const public_key_type& k : s )
   {
      ...
      try
      {
         muse::chain::verify_authority( operations, result, get_active, get_owner, get_basic, get_master_content, get_comp_content, max_recursion );
...
{ try {
   muse::chain::verify_authority( operations, get_signature_keys( chain_id ), get_active, get_owner, get_basic, get_master_content, get_comp_content, max_recursion );
