#include <muse/market_history/market_history_api.hpp>

#include <muse/chain/base_objects.hpp>

namespace muse { namespace market_history {
...
{
   public:
      market_history_api_impl( muse::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_mbd, MBD_SYMBOL ) / asset( itr->open_muse, MUSE_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_mbd, MBD_SYMBOL ) / asset( itr->close_muse, MUSE_SYMBOL ) ).to_real();
   ...
   result.muse_volume = volume.muse_volume;
   result.mbd_volume = volume.mbd_volume;
...
market_volume market_history_api_impl::get_volume() const
{
   ...
   {
      result.muse_volume.amount += itr->muse_volume;
      result.mbd_volume.amount += itr->mbd_volume;
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index_type< muse::chain::limit_order_index >().indices().get< muse::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( MBD_SYMBOL, MUSE_SYMBOL ) );
   ...
   while( itr != order_idx.end() && itr->sell_price.base.asset_id == MBD_SYMBOL && result.bids.size() < limit )
   {
      ...
      cur.muse = ( asset( itr->for_sale, MBD_SYMBOL ) * itr->sell_price ).amount;
      cur.mbd = itr->for_sale;
      result.bids.push_back( cur );
      ++itr;
   }

   itr = order_idx.lower_bound( price::max( MUSE_SYMBOL, MBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.asset_id == MUSE_SYMBOL && result.asks.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.quote.to_real() / itr->sell_price.base.to_real();
      cur.muse = itr->for_sale;
      cur.mbd = ( asset( itr->for_sale, MUSE_SYMBOL ) * itr->sell_price ).amount;
...
market_history_api::market_history_api( const muse::app::api_context& ctx )
{
