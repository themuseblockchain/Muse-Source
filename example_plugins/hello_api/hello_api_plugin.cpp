...
namespace muse { namespace example_plugin {

class hello_api_plugin : public muse::app::plugin
{
   public:
      /**
...
{
   public:
      hello_api_api( const muse::app::api_context& ctx );
   ...
   private:
      muse::app::application& _app;
      uint32_t _api_call_count = 0;
};

} }

FC_API( muse::example_plugin::hello_api_api,
   (get_message)
   )
...
hello_api_api::hello_api_api( const muse::app::api_context& ctx ) : _app(ctx.app) {}
...
/**
 * The MUSE_DEFINE_PLUGIN() macro will define a muse::plugin::create_hello_api_plugin()
 * factory method which is expected by the manifest.
 */

MUSE_DEFINE_PLUGIN( hello_api, muse::example_plugin::hello_api_plugin )
